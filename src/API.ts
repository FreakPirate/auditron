...
export const getBidsForProject = async (projectId: string): Promise<UserBid[]> => {
	const response = await fetch(`${sourceUrl}/api/bids/project/${projectId}`);
	const data = await response.json();
	return data.data;
};
...
export const updateProjectManualAuditStatus = async (
	projectId: string,
...
// /api/audit-file
export const createAuditFile = async (auditFile: AuditFile): Promise<any> => {
...
// /api/estimated-gas
export const getEstimatedGas = async (): Promise<any> => {
	const response = await fetch(`${sourceUrl}/api/estimated-gas`);
	const data = await response.json();
	return data;
};
